...
def generate_token(email, password):
    ...
    # Asking if the user wants to use a proxy
    use_proxy = input("Do you want to use a proxy? (y/n): ").strip().lower()
    proxies = None
    
    if use_proxy == 'y':
        proxy = input("Enter proxy (e.g., http://ip:port): ").strip()
        proxies = {
            'http': proxy,
            'https': proxy,
        }
    ...
    try:
        response = requests.post(url, data=form, headers=headers, proxies=proxies)
        data = response.json()
